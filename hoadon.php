...
<body>
    <div class="container-fluid">
        ...
        <ul class="nav nav-tabs">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="hoadon.php">Hóa đơn</a>
            </li>
            <li class="nav-item">
            ...
            <tbody>
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<tr>';
                        echo '<th scope="row">' . $row['Ma_hd'] . '</th>';
                        echo '<td>' . $row['Ma_nv'] . '</td>';
                        echo '<td>' . $row['Ma_kh'] . '</td>';
                        echo '<td>' . $row['Ma_h'] . '</td>';
                        echo '<td>' . $row['Tenhang_hd'] . '</td>';
                        echo '<td>' . $row['So_luong'] . '</td>';
                        echo '<td>' . $row['Gia_ban'] . '</td>';
                        echo '<td>' . $row['Gia_ban'] * $row['So_luong'] . '</td>';
                        echo '</tr>';
                    }
                }
